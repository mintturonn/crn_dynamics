...
sims_pn_crn_ti <- function(init_inf, types,  params, params_pn, crn_screen_pr, crn_pn_pr) {
  ...
  for (i in 2:(params[["simlength"]] +1)){
    for (m in 1:length(init_inf) ){
      ...
      if ( inf_state[m,i-1]==0 ) {
        ...
        diagn[m,i]     <- ifelse(crn_screen_pr[m,i] < params[["screen_pr"]], 1, 0)
      ...
      if ((diagn[m,i] >0 &  params[["pn_pr"]]>0 )>0 ) {
        ...
        pn_index[m,i] <- ifelse(crn_pn_pr[m,i] < params[["pn_pr"]], 1, 0) 
